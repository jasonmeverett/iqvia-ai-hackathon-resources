"""
Reads a shared docx file from the workflow directory and returns the document content as markdown.
"""
...
class ToolParameters(BaseModel):
    ...
    """
    filepath: str = Field(description="The workflow-relative local path to the docx file to read. example: 'report.docx'")
...
def run_tool(config: UserParameters, args: ToolParameters) -> Any:
    ...
    with open(args.filepath, "rb") as docx_file:
        result = mammoth.convert_to_markdown(docx_file)
        markdown = result.value  # This is the markdown string
